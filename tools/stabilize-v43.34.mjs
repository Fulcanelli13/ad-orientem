...
replace("textMode: state.live.textMode, language: state.language }; storage.setItem(exports.SESSION_KEY", "textMode: state.live.textMode, language: state.language, resolvedSession: globalThis.AO_ACTIVE_MASS_SESSION?.date === state.selectedDate ? globalThis.AO_ACTIVE_MASS_SESSION : (state.resolution?.proper?.status==='ready' ? {date:state.selectedDate,proper:state.resolution.proper.data,resolvedMass:globalThis.AO_CELEBRATION_API?.getResolvedMass?.()||{celebrationId:'mass_of_day',celebrationType:'calendar'},themePlan:state.resolution.colourPlan} : null) }; storage.setItem(exports.SESSION_KEY");
